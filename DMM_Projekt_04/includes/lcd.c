...
LCD_DrawLine (uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, uint8_t mode)
{
  ...
  int8_t e2;http://de.wikipedia.org/wiki/Bresenham-Algorithmus
...
void
LCD_PutNumber (volatile uint8_t number, uint8_t set)
{	
	char text_num[array];
	
	itoa(number, text_num, set);	
	LCD_PutString(text_num);
}
